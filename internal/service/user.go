...
func (s *UserService) Get(ctx context.Context, username string) (entity.User, error) {
	...
	s.l.WithContext(ctx).Debug().Msgf("UserService - Get - username: %s, error[%v]", username, err)
	if err != nil {
...
func (s *UserService) CacheGet(ctx context.Context, username string) (entity.User, error) {
	...
	if err != nil {
		if err == cache.ErrMiss {
			s.l.WithContext(ctx).Debug().Msgf("UserService - GetCache - cache miss %s", key)
		} else {
			s.l.WithContext(ctx).Warn().Msgf("UserService - GetCache - get from cache %s failed: %v", key, err)
		}
		...
		if err != nil {
			s.l.WithContext(ctx).Warn().Msgf("UserService - GetCache - set to cache %s/%+v failed: %v", key, u, err)
		}
		s.l.WithContext(ctx).Debug().Msgf("UserService - GetCache - set cache %s/%+v success", key, u)
		return u, nil
	}
	s.l.WithContext(ctx).Debug().Msgf("UserService - GetCache - cache hit %s: %+v", key, user)
	return user, nil
}

// QueryUser - 分页查询 User 信息.
func (s *UserService) Query(ctx context.Context, p entity.PageQuery, o entity.OrderQuery, u entity.UserQuery) (entity.PageResult, []entity.User, error) {
	...
	for _, u := range us {
		users = append(users, entity.ToUser(u))
	}
...
func (s *UserService) Update(ctx context.Context, in entity.User) (entity.User, error) {
	...
	if err != nil {
		s.l.WithContext(ctx).Warn().Msgf("UserService - Update - del cache %s failed: %v", key, err)
	}
...
func (s *UserService) AuthenticationPassword(ctx context.Context, username, pass string) (ok bool, reason string, err error) {
	u, err := s.db.GetUser(ctx, username)
	if err != nil {
		if err == sql.ErrNoRows {
